...
/**
 * One-time Google Drive setup for ARCHIVE_BACKEND=drive.
 *
 *   npm run drive:bootstrap
 *
 * Walks the desktop OAuth consent flow on 127.0.0.1, creates the originals
 * folder, and writes the four worker secrets to the git-ignored
 * oauth-output/drive-oauth.json. Skip this entirely when archiving to R2.
 */
...
if (!tokens.refresh_token || !tokens.access_token) {
	throw new Error(
		"Google nie zwrócił refresh tokenu. Cofnij dostęp i spróbuj ponownie.",
	);
}
...
if (!folderResponse.ok) {
	throw new Error("Nie udało się utworzyć folderu Drive.");
}
...
console.log(
	"Wgraj te cztery wartości jako sekrety workera i ustaw ARCHIVE_BACKEND=drive.",
);
