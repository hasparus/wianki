...
through `workers/archive` into whichever archive the deployment chose: a
Cloudflare R2 bucket (`ARCHIVE_BACKEND=r2`, the default) or the couple's
private Google Drive folder (`ARCHIVE_BACKEND=drive`).
...
moderation retry, archive reconciliation, and confirmed deletion actions.
Deletion can partially fail, so Supabase deletion and archive removal remain
retryable.
...
boundary), R2 or Google Drive (private originals), Google Vision (moderation),
and
...
- Originals stream through the Cloudflare Worker into the configured archive.
  One deployment writes to one archive; both backends live behind the same
  `ArchiveBackend` contract and the same signed-receipt boundary.
...
- `workers/archive` owns archive credentials and archive API calls.
