...
export function UploadPanel({ onComplete }: { onComplete: () => void }) {
	...
	async function processOne(index: number, init: InitUpload) {
		...
		try {
			...
			const hotPromise = supabaseBrowser()
			...
			const [hot, archiveResponse] = await Promise.all([
				hotPromise,
				archivePromise,
			]);
			...
			if (!finalize.ok)
				throw new Error(result.error ?? "Finalizacja nie powiodła się.");
			if (result.warning) {
				update(index, {
	...
	async function retryArchive(index: number) {
		...
		try {
			...
			const tokenResponse = await fetch(
				`/api/uploads/${item.photoId}/archive`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
		...
		try {
			...
			async function worker() {
				while (next < freshUploads.length) {
					...
					results.push(
						await processOne(
							freshIndices[localIndex],
							freshUploads[localIndex],
						),
					);
				}
			}
