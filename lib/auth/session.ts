...
export async function readGuestSession(
  ...
  const payload = await verifySession(cookieToken, serverEnv().GUEST_SESSION_SECRET);
  if (payload?.role !== "guest" || typeof payload.guestId !== "string") return null;
...
export async function readAdminSession(
  ...
  const payload = await verifySession(token ?? cookieToken, serverEnv().ADMIN_SESSION_SECRET);
