...
The photograph feed is a full-viewport-width horizontal rail, not a page-length grid. Its section is exactly one dynamic viewport tall, and the rail fills all space left below the gallery controls. Guests swipe sideways through it; a mouse wheel follows the same axis while the rail can still move. Pinching or using the plain minus/plus icons changes whether one, two, or three rows share the rail without changing its outer height. A pinch crosses a deliberate threshold, then Motion crossfades the complete old and new column-major layout layers without scaling or sliding either one. Button zoom keeps the nearest middle photograph in the same viewport position; a pinch anchors the photograph beneath its focal point. Set the new scroll offset before paint and counter-shift the outgoing layer so the old view does not move. Individual photographs never fly between slots or cross one another; never delegate the transition to CSS Grid reflow or animate plates independently between row levels. One row preserves each photograph's proportion; denser views use evenly sized cropped plates that expand to fill an otherwise short rail. The photograph at the gesture's focal point stays anchored when density changes. Fresh photos from polling reserve complete logical columns, so an existing photograph never changes rows; the first visible plate stays at the same viewport position while the new column arrives before it. Lists — upload queue, admin queue, challenge tasks — are hairline-divided rows, never cards.
...
- **Don't** inset the photograph feed with the page content or let it reflow vertically. It spans the viewport and fills the remainder of its one-screen section below the controls; density changes happen inside that stable height and keep the gesture's focal photograph anchored.
